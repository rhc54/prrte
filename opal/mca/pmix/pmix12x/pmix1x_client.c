...
int pmix1_client_init(opal_list_t *ilist)
{
    ...
    if (NULL != getenv(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        mca_pmix_pmix12x_component.native_launch = true;
    ...
    opal_list_append(&mca_pmix_pmix12x_component.jobids, &job->super);
                ...
                opal_list_t *procs)
{
    ...
    if (NULL != procs && 0 < (cnt = opal_list_get_size(procs))) {
        ...
        OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
            ...
            job = NULL;
            OPAL_LIST_FOREACH(jptr, &mca_pmix_pmix12x_component.jobids, opal_pmix1_jobid_trkr_t) {
...
int pmix1_lookup(opal_list_t *data, opal_list_t *info)
{
    ...
    if (PMIX_SUCCESS == ret) {
        ...
        n=0;
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            if (mca_pmix_pmix12x_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
                    ...
                    void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        /* look thru our list of jobids and find the
     * corresponding nspace */
        OPAL_LIST_FOREACH(job, &mca_pmix_pmix12x_component.jobids, opal_pmix1_jobid_trkr_t) {
