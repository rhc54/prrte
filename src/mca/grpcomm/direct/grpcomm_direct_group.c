...
for (i=0; i < cd->nprocs; i++) {
    pmix_output(0, "PROCS[%lu]: %s", i, PMIX_NAME_PRINT(&cd->procs[i]));
}
                                     ...
                                     prte_rml_tag_t tag, void *cbdata)
{
    ...
    rc = PMIx_server_collect_job_info(finalmembership, nfinal, &dbuf);
    if (PMIX_SUCCESS == rc) {
...
static prte_grpcomm_group_t *get_tracker(prte_grpcomm_direct_group_signature_t *sig, bool create)
{
    ...
    PMIX_LIST_FOREACH(coll, &prte_mca_grpcomm_direct_component.group_ops, prte_grpcomm_group_t) {
        ...
        if (0 == strcmp(sig->groupID, coll->sig->groupID) &&
            ...
            if (0 < sig->naddmembers) {
                pmix_output(0, "ADDREGATING MEMBERS");
    ...
    pmix_output(0, "EXISTING COLL WITH %lu ADDMBRS", coll->sig->naddmembers);
                }
            }
    ...
    pmix_output(0, "NEW COLL WITH %lu ADDMBRS", coll->sig->naddmembers);
                       ...
                       pmix_rank_t **dmns, size_t *ndmns)
{
    ...
    for (n = 0; n < sig->nmembers; n++) {
        pmix_output(0, "GET JOB FOR %s", sig->members[n].nspace);
