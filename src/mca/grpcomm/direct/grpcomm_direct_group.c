...
int prte_grpcomm_direct_grp_construct(prte_grpcomm_coll_t *coll,
                                      prte_pmix_grp_caddy_t *cd)
{
    ...
    PMIX_OUTPUT_VERBOSE((1, prte_grpcomm_base_framework.framework_output,
                         "%s grpcomm:direct:grp_construct",
    ...
    if (0 < coll->sig->bootstrap || NULL == coll->sig->signature) {
        PMIX_OUTPUT_VERBOSE((1, prte_grpcomm_base_framework.framework_output,
                             "%s grpcomm:direct:grp_construct sending to HNP",
                             PRTE_NAME_PRINT(PRTE_PROC_MY_NAME)));

        PRTE_RML_SEND(rc, PRTE_PROC_MY_HNP->rank, relay,
                      PRTE_RML_TAG_GRP_CONSTRUCT_DIRECT);
        return rc;
    }
