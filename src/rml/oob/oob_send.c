...
void prte_oob_send_nb(int fd, short args, void *cbdata)
{
    ...
    if (NULL == pr) {
        ...
        /* the URI for the target daemon might be in the database,
         * so check there next - if it is, the peer object will be added
         * to our hash table. However, we don't want to block if we
         * don't have it, so indicate it is optional
         */
        ...
        if (PRTE_SUCCESS == rc) {
             ...
             * be able to get to it via routing
             */
            reachable = false;
            pr = NULL;
