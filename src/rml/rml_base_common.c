...
#    if defined(TCP_KEEPALIVE)
    /* set the idle time */
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPALIVE, &prte_rml_base.keepalive_time,
                   sizeof(prte_rml_base.keepalive_time))
        < 0) {
        pmix_output_verbose(5, prte_rml_base.rml_output,
...
#    elif defined(TCP_KEEPIDLE)
    /* set the idle time */
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPIDLE, &prte_rml_base.keepalive_time,
                   sizeof(prte_rml_base.keepalive_time))
        < 0) {
        pmix_output_verbose(5, prte_rml_base.rml_output,
...
#    if defined(TCP_KEEPINTVL)
    ...
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPINTVL, &prte_rml_base.keepalive_intvl,
                   sizeof(prte_rml_base.keepalive_intvl))
        < 0) {
        pmix_output_verbose(5, prte_rml_base.rml_output,
...
#    if defined(TCP_KEEPCNT)
    /* set the miss rate */
    if (setsockopt(sd, IPPROTO_TCP, TCP_KEEPCNT, &prte_rml_base.keepalive_probes,
                   sizeof(prte_rml_base.keepalive_probes))
        < 0) {
        pmix_output_verbose(5, prte_rml_base.rml_output,
...
void prte_rml_set_socket_options(int sd)
{
    ...
    if (prte_rml_base.tcp_sndbuf > 0
        && setsockopt(sd, SOL_SOCKET, SO_SNDBUF, (char *) &prte_rml_base.tcp_sndbuf,
                      sizeof(int))
               < 0) {
        pmix_output_verbose(5, prte_rml_base.rml_output,
    ...
    if (prte_rml_base.tcp_rcvbuf > 0
        && setsockopt(sd, SOL_SOCKET, SO_RCVBUF, (char *) &prte_rml_base.tcp_rcvbuf,
                      sizeof(int))
               < 0) {
        pmix_output_verbose(5, prte_rml_base.rml_output,
    ...
    if (0 < prte_rml_base.keepalive_time) {
        set_keepalive(sd);
    }
}

prte_rml_peer_t *prte_rml_peer_lookup(const pmix_proc_t *name)
{
    prte_rml_peer_t *peer;

    PMIX_LIST_FOREACH(peer, &prte_rml_base.peers, prte_rml_peer_t)
    {
...
char *prte_rml_state_print(prte_rml_state_t state)
{
    switch (state) {
    case RML_UNCONNECTED:
        return "UNCONNECTED";
    case RML_CONNECTION_CLOSED:
        return "CLOSED";
    case RML_RESOLVE:
        return "RESOLVE";
    case RML_CONNECTING:
        return "CONNECTING";
    case RML_CONNECT_ACK:
        return "ACK";
    case RML_CONNECTED:
        return "CONNECTED";
    case RML_CONNECTION_FAILED:
        return "FAILED";
    default:
        return "UNKNOWN";
    }
