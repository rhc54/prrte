...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < warmup + iters; i++) {
        ...
        /* NEIGHBORS -- what the ring bargain actually costs.
         *
         * The collect fence above hands every rank the whole job's data; this
         * phase asks instead for only the two values a ring exchange would
         * have delivered, from a job that has just barriered WITHOUT
         * collecting.  Those gets are answered by direct modex: the local
         * daemon does not hold the peer's data, so it fetches it from the
         * daemon that does.  That is the comparison Slurm's PMIX_Ring is an
         * argument for - O(1) per peer you actually need, against O(N) to
         * everybody - expressed in machinery PRRTE already has.
         *
