 ...
 *
 * That makes a bare MPI_Init/MPI_Finalize a *bad* probe for a fence movement:
 * it never touches a remote peer, so it measures the fence and nothing else,
 * and every movement looks identical.  What separates them is FIRST TOUCH,
 * and the two patterns that bracket it:
 *
 *   --ring   each rank sendrecv's with its two neighbours only.  A movement
 *            that shares sideways should already hold both.
